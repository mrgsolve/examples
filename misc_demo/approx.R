...
##' - `$PREAMBLE` gets called __ONCE__ we set thing up there
##'     - We `mrgx::get` some `Rcpp::NumericVector`s (`x` and `y`) from the model `$ENV`
##'     - Note that the call is `mrgx::get<Rcpp::NumericVector>`; `mrgx::get` is a templated function, so we need to 
##'     say what type we are working with.
##' - Now, in `$MAIN` we can call the `appx` function and pass in `x`, `y`, and `xout`.
##'     - `appx` returns the result as a `Rcpp::List`.  We're interested in the `y` element in that list, 
##'     so we need to get specific about what type (`double`) that needs to be.
##' 
##' # More info
##' 
